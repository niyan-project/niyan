...
On the first interactive `niyan add` in a repository without a root `.gitattributes` file, Niyān asks which Git-visible file formats should use Git LFS and records the selected repository-wide rules before staging. Git supplies the file list and ignore handling; Niyān groups names by extension without opening or classifying their contents. Compressed compound formats such as `.nii.gz` and `.tar.zst` remain distinct choices. Once `.gitattributes` exists, Niyān treats it as the repository policy and skips discovery entirely. Non-interactive calls also skip the selector, while `--lfs` and `--git` remain available for explicit standard `.gitattributes` overrides.

Interactive staging shows a terminal status indicator by default. Add `--verbose` to replace it with Git's per-file output.

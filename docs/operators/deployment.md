...
The script downloads only `compose.yml`, `Caddyfile`, `.env.example`, and the deployment runbook from the same `v0.3.0` tag, then creates a private `.env` for editing. It refuses to overwrite a non-empty destination and does not invoke `sudo`, start Docker, or modify the host outside `niyan-deploy`.

To choose a different empty destination, pass it after `sh -s --`:

```shell
curl --proto '=https' --tlsv1.2 -fsSL https://raw.githubusercontent.com/niyan-project/niyan/v0.3.0/deploy/install.sh | sh -s -- /srv/niyan
cd /srv/niyan
```

...
class LfsBatchApiTests(TestCase):
    ...
    def test_basic_authentication_scope_boundary_and_current_role_are_enforced(self):
        ...
        _, dataset_token = create_access_token(user=self.user, name='This dataset', scopes=['write_repository'], origin=AccessToken.Origin.CLI, dataset=self.dataset)
        ...
        allowed = self.post_batch({'operation': 'upload', 'objects': []}, token=dataset_token)
        expired = self.post_batch({'operation': 'upload', 'objects': []}, token=expired_token)
        revoked = self.post_batch({'operation': 'upload', 'objects': []}, token=revoked_token)
